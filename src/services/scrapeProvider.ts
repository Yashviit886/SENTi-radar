...
async function fetchFromX(
  ...
  try {
    ...
    if (html.includes('Log in to X') && !html.toLowerCase().includes('data-testid="tweet"')) {
...
async function fetchFromReddit(
  ...
  try {
    ...
    if (html.includes('Are you a human?') || html.toLowerCase().includes('are you a human') || (!html.includes('reddit') && html.length < 5000)) {
      return { posts: [], status: 'blocked' };
    }
